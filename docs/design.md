...
## 4. 전 버전 지원 — ViaProxy 샌드위치

버전별 봇 구현을 각각 만드는 것은 유지 불가능하다. 대신 번역 계층을 끼운다.

```
봇 (MCProtocolLib, 프로토콜 1개 고정)
      ↓
ViaProxy (버전 번역)
      ↓
대상 서버 (Classic ~ 최신, Bedrock 포함)
```

ViaProxy는 Classic·Alpha·Beta·릴리스 전 구간과 Bedrock까지 커버하며, 단독 실행뿐 아니라 **라이브러리로 임베드**할 수 있다(ViaProtocolHack 계열). 봇 코드는 최신 프로토콜 하나만 알면 되고, 버전 대응 부담은 Via 업스트림이 진다.

### 한계와 완화

Via 번역은 무손실이 아니다. 구버전 고유의 패킷 레벨 버그가 가려지거나, 반대로 Via가 없던 문제를 만들 수 있다.

**완화**: 정말 보증해야 하는 2~3개 버전은 네이티브 프로토콜 봇으로 이중 검증한다. 나머지 버전은 Via로 넓게 훑는다. `versions.yaml`에서 버전별로 `native: true|false`를 명시한다.

디버깅 시 **"Via가 원인일 가능성"을 항상 후보에 두어야 한다.** 특정 버전에서만 재현되는 문제는 네이티브 봇으로 교차 확인한 뒤에야 플러그인 버그로 확정한다.
...
bot/
  ...
  bot-via/           ViaProxy 임베드
orchestrator/        Docker 기동/월드 리셋/버전 매트릭스
...
버전 매트릭스는 코드가 아니라 `versions.yaml`이다. 버전 추가가 설정 한 줄로 끝나야 한다.

```yaml
versions:
  - id: "1.20.4"
    server: { image: "itzg/minecraft-server", version: "1.20.4", type: "PAPER" }
    bot: { protocol: 765, via: false }
    native: true          # 네이티브 프로토콜 검증 대상
  - id: "1.16.5"
    server: { image: "itzg/minecraft-server", version: "1.16.5", type: "PAPER" }
    bot: { protocol: 765, via: true }
    native: false         # Via 경유
```
