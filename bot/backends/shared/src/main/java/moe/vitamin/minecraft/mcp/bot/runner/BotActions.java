...
public final class BotActions {
    ...
    public BotActions moveTo(double x, double y, double z) {
        require();
        // Through PlayerSync because the movement packet's shape is version-dependent — 1.21.2
        // added the horizontal-collision flag — and that difference belongs in the one file a
        // backend overrides, not in two.
    ...
    public BotActions closeMenu() {
        ...
        if (containerId != BotSession.NO_CONTAINER) {
            ...
            // The server acts on a client-requested close without echoing it, so nothing else
            // will ever clear this. Leaving it set made `menu()` keep reporting a screen the
